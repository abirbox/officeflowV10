...
const DispatchCalendarPage = () => {
  ...
  const range = useMemo(() => {
    ...
    return {
      from: iso(startOfMonth(cursor)),
      to: iso(endOfMonth(cursor)),
    };
  }, [view, cursor]);
  ...
  useEffect(() => {
    ...
    const loadFilterOptions = async () => {
      try {
        const [clientsRes, vendorsRes, officersRes, postSitesRes] = await Promise.all([
        ...
        setPostSites(Array.isArray(postSitesRes.data) ? postSitesRes.data : []);
      } catch (e) {
        if (!cancelled) {
  ...
  // Load schedules whenever date range or filters change.
  useEffect(() => {
    if (!canView) {
      setLoading(false);
      return;
    ...
    if (filters.post_pin) params.post_pin = filters.post_pin;

    api.get('/dispatch/schedules', { params })
  ...
  if (!canView) {
    return (
      <div className="p-8 text-[#64748B]">
        You do not have permission to view the Dispatch Calendar.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6" data-testid="dispatch-calendar-page">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[#64748B] mt-1">
            {rows.length} scheduled shift{rows.length !== 1 && 's'} in view
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-[#18181B] border border-[#E2E8F0] dark:border-[#27272A] rounded-xl p-4">
        <div className="flex items-center justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-xs text-[#64748B] mt-0.5">
              Filter schedules by client, vendor, officer or post pin.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <Select
            ...
            onValueChange={(value) =>
              updateFilter('client_id', value)
            }
          >
            <SelectTrigger data-testid="calendar-client-filter">
          ...
          <Select
            ...
            onValueChange={(value) =>
              updateFilter('vendor_id', value)
            }
          >
            <SelectTrigger data-testid="calendar-vendor-filter">
              <SelectValue placeholder="By Vendor" />
            </SelectTrigger>
          ...
          <Select
            ...
            onValueChange={(value) =>
              updateFilter('officer_id', value)
            }
          >
            <SelectTrigger data-testid="calendar-officer-filter">
              <SelectValue placeholder="By Officer" />
            </SelectTrigger>
          ...
          {/* Post Pin */}
          <Select
            value={filters.post_pin || 'all'}
            onValueChange={(value) =>
              updateFilter('post_pin', value)
            }
          >
            <SelectTrigger data-testid="calendar-post-pin-filter">
              <SelectValue placeholder="By Post Pin" />
            </SelectTrigger>

            <SelectContent>
              <SelectItem value="all">All Post Pins</SelectItem>

              {postSites.map((post) => (
                <SelectItem
                  key={post.id}
                  value={String(post.post_pin || '')}
                  disabled={!post.post_pin}
                >
                  {formatPin(post) || post.post_pin || 'No Pin'}
                  {post.name ? ` — ${post.name}` : ''}
                </SelectItem>
              ))}
            </SelectContent>
      ...
      {/* Current period */}
      <div className="text-lg font-semibold text-[#0F172A] dark:text-[#FAFAFA]">
        {label}
      </div>
          ...
          return (
            <div
              ...
              className={`min-h-[110px] p-2 ${
                inMonth
                  ? ''
                  : 'bg-[#FAFAFA] dark:bg-[#0F0F11] opacity-60'
              }`}
              ...
              <div className="space-y-1">
                {events.slice(0, 3).map((e) => (
                  <button
                    key={e.id}
                    onClick={() => onSelect(e)}
                    className={`w-full text-left px-1.5 py-1 rounded text-[11px] truncate ${
                      CONFIRM_BADGE[e.confirmation_status] ||
    ...
    {events.length === 0 ? (
      ...
      events.map((e) => (
        <button
          key={e.id}
          onClick={() => onSelect(e)}
          className="w-full text-left p-4 hover:bg-[#F8FAFC] dark:hover:bg-[#0F0F11] transition"
          data-testid={`cal-event-${e.id}`}
        >
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-[#0F172A] dark:text-[#FAFAFA]">
                {e.post_site_name || '—'}
              </div>

              <div className="text-xs text-[#64748B] mt-1">
                {e.officer_name} · {e.client_name} · {e.vendor_name} · Post Pin:{' '}
                {formatPin(e) || '—'}
              </div>
            </div>
...
const ScheduleDetailDialog = ({ row, onClose }) => {
  ...
  return (
    ...
    >
      <DialogContent
        className="max-w-lg"
        data-testid="cal-detail-dialog"
      >
        <DialogHeader>
          <DialogTitle>Schedule Details</DialogTitle>
          <DialogDescription>
            Overview of the selected dispatch schedule
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-2 text-sm">
          ...
          <Row label="Time">
            {row.start_time} – {row.end_time} ({row.duty_hours}h)
          </Row>
          ...
          <Row label="Post Site">
            {formatPin(row)
              ? `${formatPin(row)} — ${row.post_site_name || '—'}`
              : row.post_site_name || '—'}
          </Row>
          ...
          <Row label="Confirmation">
            <span
              className={`px-2 py-0.5 rounded-full text-xs ${
                CONFIRM_BADGE[row.confirmation_status] ||
                'bg-slate-100'
              }`}
            >
          ...
          {row.billing_rate != null && (
            <Row label="Billing Rate">
              {row.billing_rate}
            </Row>
          )}
...
const Row = ({ label, children }) => (
  <div className="grid grid-cols-3 gap-2">
    ...
    <div className="col-span-2 text-[#334155] dark:text-[#E4E4E7]">
      {children}
    </div>
  </div>
);
