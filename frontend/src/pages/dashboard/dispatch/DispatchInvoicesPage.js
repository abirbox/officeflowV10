...
const DispatchInvoicesPage = () => {
  ...
  const isClientPortal = String(scopeBase || '').includes('/portal');
            ...
            {invoices.map((inv) => (
              <tr key={inv.id} data-testid={`invoice-row-${inv.id}`}>
                <td className="px-3 py-2 font-mono">
                  {inv.invoice_number}
                  {!isClientPortal && inv.generated_by_role === 'client' && inv.generated_by_client_code && (
                    <div className="mt-1">
                      <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300"
                        title="Client-generated invoice"
                        data-testid={`invoice-client-badge-${inv.id}`}>
                        {inv.generated_by_client_code}
                      </span>
                    </div>
                  )}
                </td>
