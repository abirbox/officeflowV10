...
import { Plus, Filter, X, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, MoreVertical, Columns as ColumnsIcon, Download, GripVertical, Eye, EyeOff, Upload, Pencil, Check } from 'lucide-react';
...
const COLUMN_CATALOG = [
  ...
  { key: 'post_pin',    label: 'Post Site Pin',        csv: (r) => r.post_pin ? `${r.vendor_code ? `${r.vendor_code} ` : ''}# ${r.post_pin}` : '' },
  ...
  { key: 'remarks',     label: 'Remarks',              csv: (r) => {
      const a = String(r.remarks || '').split('\n').map((s) => s.trim()).filter(Boolean);
      const b = String(r.last_modified_remarks || '').split('\n').map((s) => s.trim()).filter(Boolean);
      const seen = new Set(a); const merged = [...a];
      for (const s of b) { if (!seen.has(s)) { seen.add(s); merged.push(s); } }
      return merged.join('; ');
    } },
...
// Stable defaults keep the schedule readable instead of squeezing every
// column equally. Dragging a header edge still overrides these widths.
const DEFAULT_COLUMN_WIDTHS = {
  date: 142, shift: 108, start_time: 96, end_time: 96, duty_hours: 92,
  duty_rate: 112, billing_rate: 124, site: 220, city: 132, post_pin: 132,
  officer: 188, shift_status: 166, confirmation: 178, confirmed_by: 194,
  remarks: 220, client: 190, vendor: 190,
};
const MANAGE_COLUMN_WIDTH = 96;
...
// Build a styled .xlsx workbook that mirrors the on-screen schedule table
// (pink header band, tinted "Date" column, red bold post pin cells, green
// city cells, black-bold date cells). We do this client-side because visible
// columns are chosen per-user and we want to export exactly what the user
// sees on-screen.
async function buildScheduleXlsx({ headers, rows, columnKeys, filename }) {
  const ExcelJS = (await import('exceljs')).default;
  const wb = new ExcelJS.Workbook();
  wb.creator = 'OfficeFlow';
  const ws = wb.addWorksheet('Dispatch Schedule', {
  ...
  headerRow.eachCell((cell) => {
    ...
    cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFBC9FF' } };
  ...
  rows.forEach((row) => {
    ...
    r.eachCell((cell, colNumber) => {
      ...
      if (key === 'date') {
        ...
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE8FAFF' } };
      } else if (key === 'post_pin') {
        cell.font = { bold: true, size: 11, color: { argb: 'FFDC2626' } };
      } else if (key === 'city') {
        cell.font = { bold: true, size: 11, color: { argb: 'FF059669' } };
      } else if (key === 'duty_rate' || key === 'billing_rate' || key === 'duty_hours') {
  ...
  const widthByKey = {
    date: 20, shift: 14, start_time: 12, end_time: 12, duty_hours: 12,
    duty_rate: 14, billing_rate: 16, site: 28, city: 16, post_pin: 16,
    officer: 24, shift_status: 20, confirmation: 22, confirmed_by: 26,
    remarks: 32, client: 24, vendor: 24,
  };
  columnKeys.forEach((k, i) => { ws.getColumn(i + 1).width = widthByKey[k] || 16; });
...
const DispatchSchedulePage = ({ todayOnly = false }) => {
  ...
  const emptySiteForm = { post_pin: '', name: '', location: '', city: '', type: 'Unarmed', notes: '' };
  ...
  const [remarkEditId, setRemarkEditId] = useState(null);
  const [remarkDraft, setRemarkDraft] = useState('');
  const [remarkSaving, setRemarkSaving] = useState(false);
  const [remarkLineEdit, setRemarkLineEdit] = useState(null); // { rowId, idx }
  const [remarkLineDraft, setRemarkLineDraft] = useState('');
  const [filtersOpen, setFiltersOpen] = useState(false);

  const getRemarksList = (r) => {
    // Combine remarks from every source: creation + inline edits (stored in
    // `remarks`) and the latest status/confirmation remark (`last_modified_remarks`).
    const fromRemarks = String(r.remarks || '').split('\n').map((s) => s.trim()).filter(Boolean);
    const fromStatus = String(r.last_modified_remarks || '').split('\n').map((s) => s.trim()).filter(Boolean);
    const seen = new Set(fromRemarks);
    const merged = [...fromRemarks];
  ...
  const addRemark = async (r) => {
    ...
    if (!text) { setRemarkEditId(null); setRemarkDraft(''); return; }
    const lines = getRemarksList(r);
    const combined = [...lines, text].join('\n');
    setRemarkSaving(true);
    try {
      await persistRemarks(r, combined);
      setRemarkDraft('');
      setRemarkEditId(null);
      toast.success('Remark added');
    } catch (e) {
      toast.error(formatApiErrorDetail(e.response?.data?.detail) || 'Failed to add remark');
    } finally { setRemarkSaving(false); }
  };

  const removeRemarkLine = async (r, idx) => {
    const next = getRemarksList(r).filter((_, i) => i !== idx).join('\n');
    try {
      await persistRemarks(r, next);
    } catch (e) {
      toast.error(formatApiErrorDetail(e.response?.data?.detail) || 'Failed to remove remark');
    }
  };

  const saveRemarkLine = async (r, idx) => {
    const text = remarkLineDraft.trim();
    const lines = getRemarksList(r);
    const next = (text
      ? lines.map((l, i) => (i === idx ? text : l))
      : lines.filter((_, i) => i !== idx)
    ).join('\n');
    setRemarkSaving(true);
    try {
      await persistRemarks(r, next);
      setRemarkLineEdit(null);
      setRemarkLineDraft('');
      toast.success(text ? 'Remark updated' : 'Remark removed');
    } catch (e) {
      toast.error(formatApiErrorDetail(e.response?.data?.detail) || 'Failed to update remark');
    } finally { setRemarkSaving(false); }
  };
  ...
  const [columnConfig, setColumnConfig] = useState(() => {
    try {
      ...
      // Corrupt/legacy JSON — log and fall through to defaults.
      console.warn('DispatchSchedulePage: could not read column config from localStorage', e);
    }
  ...
  useEffect(() => {
    try {
      ...
      // Quota errors etc. — non-fatal but worth logging.
      console.warn('DispatchSchedulePage: could not persist column config', e);
    }
  ...
  // ---- Column widths (resizable by dragging the header edge) --------------
  const widthsKey = `dispatch.schedule.colWidths.${user?.id || 'anon'}`;
  const [colWidths, setColWidths] = useState(() => {
    try {
      const raw = localStorage.getItem(widthsKey);
      if (raw) return JSON.parse(raw);
    } catch (e) {
      console.warn('DispatchSchedulePage: could not read column widths', e);
    }
    return {};
  });
  useEffect(() => {
    try {
      ...
      console.warn('DispatchSchedulePage: could not persist column widths', e);
    }
  }, [colWidths, widthsKey]);

  // ---- Row density (compact / comfortable) — persisted per user ----------
  const densityKey = `dispatch.schedule.density.${user?.id || 'anon'}`;
  const [density, setDensity] = useState(() => {
    try {
      const v = localStorage.getItem(densityKey);
      if (v === 'compact' || v === 'comfortable') return v;
    } catch (e) {
      console.warn('DispatchSchedulePage: could not read density preference', e);
    }
    return 'comfortable';
  });
  useEffect(() => {
    try {
      ...
      console.warn('DispatchSchedulePage: could not persist density preference', e);
    }
  }, [density, densityKey]);
  ...
  const startResize = (key, e) => {
    ...
    const startW = th ? th.getBoundingClientRect().width : 140;
    const onMove = (ev) => {
      const w = Math.max(60, Math.min(800, startW + (ev.clientX - startX)));
  ...
  const resetColumns = () => { setColumnConfig(defaultConfig); setColWidths({}); setDensity('comfortable'); };
  ...
  const exportExcel = async () => {
    ...
    try {
      await buildScheduleXlsx({
        headers, rows: body, columnKeys,
        filename: `dispatch-schedule-${stamp}.xlsx`,
      });
      toast.success(`Exported ${rows.length} row${rows.length === 1 ? '' : 's'} to Excel`);
    } catch (e) {
      toast.error('Could not build Excel file');
    }
  };
  ...
  useEffect(() => {
    api.get('/dispatch/clients')
      .then(r => setClients(r.data))
      .catch(e => console.warn('Dispatch clients failed:', e.response?.status, e.response?.data));

    api.get('/dispatch/vendors')
      .then(r => setVendors(r.data))
      .catch(e => console.warn('Dispatch vendors failed:', e.response?.status, e.response?.data));

    api.get('/dispatch/officers')
      .then(r => setOfficers(r.data))
      .catch(e => console.warn('Dispatch officers failed:', e.response?.status, e.response?.data));

    api.get('/dispatch/post-sites')
      .then(r => setPostSites(r.data))
      .catch(e => console.warn('Dispatch post-sites failed:', e.response?.status, e.response?.data));
  }, []);
  ...
  const downloadImportTemplate = async () => {
    try {
      const res = await api.get('/dispatch/schedules/import-template', { responseType: 'blob' });
      const url = URL.createObjectURL(new Blob([res.data], { type: 'text/csv' }));
      const a = document.createElement('a');
      a.href = url;
      a.download = 'dispatch-schedule-template.csv';
      document.body.appendChild(a); a.click(); a.remove();
      URL.revokeObjectURL(url);
  ...
  const runImport = async ({ dryRun = false } = {}) => {
    ...
    try {
      ...
      const { data } = await api.post('/dispatch/schedules/import', fd, {
      ...
      if (dryRun) {
        toast.info(`Preview: ${data.created} ready · ${skipped} skipped · ${errs} error${errs === 1 ? '' : 's'}`);
      } else if (data.created > 0) {
        toast.success(
          `Imported ${data.created} schedule${data.created === 1 ? '' : 's'}` +
          (skipped ? ` · ${skipped} skipped` : '')
        );
        load();
      } else {
        toast.error('No schedules were imported — see details below');
      }
    } catch (e) {
  ...
  const submit = async () => {
    ...
    if (editing) {
      ...
      try {
        ...
        Object.entries(form).forEach(([k, v]) => {
          // Multiple-date helper fields are never part of an edit request.
          if (k === 'date_from' || k === 'date_to') return;
    ...
    // Create validation.
    for (const k of ['shift_type', 'start_time', 'end_time', 'client_id', 'vendor_id', 'post_site_id', 'officer_id']) {
      if (!form[k]) {
        toast.error(`${k.replace(/_/g, ' ')} is required`);
        return;
      }
    ...
    try {
      ...
      // Do not send unused date fields.
      if (scheduleMode === 'once') {
      ...
      const { data } = await api.post('/dispatch/schedules', payload);

      if (scheduleMode === 'multiple') {
        const count = data?.created || data?.items?.length || 0;
        toast.success(
          `${count} schedule${count === 1 ? '' : 's'} created successfully`
        );
      } else {
  ...
  const createInlinePostSite = async () => {
    for (const k of ['post_pin', 'name', 'location']) {
      if (!siteForm[k]) { toast.error(`${k.replace(/_/g, ' ')} is required`); return; }
    }
    try {
      const { data } = await api.post('/dispatch/post-sites', siteForm);
      const list = await api.get('/dispatch/post-sites');
      setPostSites(list.data);
      // Auto-select the newly created site (client/vendor stay chosen from the schedule form itself)
      setForm((prev) => ({ ...prev, post_site_id: data.id }));
      setSiteDialogOpen(false);
      setSiteForm(emptySiteForm);
      toast.success('Post site created');
  ...
  const applyStatus = async () => {
    ...
    try {
      ...
      if (status === 'Clocked In') payload.actual_check_in = now;
      if (status === 'Clocked Out') payload.actual_check_out = now;
  ...
  return (
    <div className="space-y-6" data-testid="dispatch-schedule-page">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button
            ...
            data-testid="column-chooser-btn"
            className="h-9"
          >
      ...
      {/* Table */}
      <div className="max-w-full bg-white dark:bg-[#18181B] border border-[#E2E8F0] dark:border-[#27272A] rounded-xl overflow-x-auto overscroll-x-contain">
        <table
          className={`w-full ${tableTextSize} table-fixed border-separate border-spacing-0`}
            ...
            : rows.map((r, index) => {
              ...
              const cellFor = (key) => {
                switch (key) {
                  ...
                  case 'post_pin':     return <span className="font-mono text-xs">{r.post_pin ? `${r.vendor_code ? `${r.vendor_code} ` : ''}# ${r.post_pin}` : '—'}</span>;
                  case 'officer': {
                    const isTemporaryOfficer = r.officer_id === 'TEMP';
                    const isOpenShift = r.officer_id === 'OPEN_SHIFT';

                    if (isTemporaryOfficer) {
                      return (
                        <span className="inline-flex items-center rounded px-2 py-1 font-semibold bg-yellow-300 text-black">
                          Temporary Officer
                        </span>
                      );
                    }
                    ...
                    if (isOpenShift) {
                      return (
                        <span className="inline-flex items-center rounded px-2 py-1 font-semibold bg-sky-300 text-black">
                          Open Shift
                        </span>
                      );
                    }

                    return r.officer_name || '—';
                  }
                  case 'shift_status':
                    return canEdit ? (
                      <Select
                      ...
                      <div className="max-w-[240px]" data-testid={`sched-remarks-${r.id}`}>
                        {list.length > 0 ? (
                          <div className="flex flex-col gap-1 mb-1">
                            {list.map((rm, i) => {
                              const lineEditing = remarkLineEdit && remarkLineEdit.rowId === r.id && remarkLineEdit.idx === i;
                              return (
                                <div key={i} className="group flex items-start gap-1 rounded bg-[#F1F5F9] dark:bg-[#27272A] px-1.5 py-1 text-xs text-[#334155] dark:text-[#E4E4E7]" data-testid={`sched-remark-item-${r.id}-${i}`}>
                                  {lineEditing ? (
                                    <div className="flex items-center gap-1 flex-1">
                                      <Input
                                        autoFocus
                                        value={remarkLineDraft}
                                        onChange={(e) => setRemarkLineDraft(e.target.value)}
                                        onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); saveRemarkLine(r, i); } if (e.key === 'Escape') { setRemarkLineEdit(null); setRemarkLineDraft(''); } }}
                                        className="h-6 text-xs"
                                        disabled={remarkSaving}
                                        data-testid={`sched-remark-edit-input-${r.id}-${i}`}
                                      />
                                      <button type="button" onClick={() => saveRemarkLine(r, i)} disabled={remarkSaving} className="text-[#16A34A] hover:text-[#15803D] shrink-0" title="Save" data-testid={`sched-remark-edit-save-${r.id}-${i}`}><Check className="w-3.5 h-3.5" /></button>
                                      <button type="button" onClick={() => { setRemarkLineEdit(null); setRemarkLineDraft(''); }} className="text-[#94A3B8] hover:text-[#334155] shrink-0" title="Cancel"><X className="w-3.5 h-3.5" /></button>
                                    </div>
                                  ) : (
                                    <>
                                      <span
                                        className="flex-1 whitespace-pre-wrap break-words cursor-text"
                          ...
                          !editing && <span className="text-[#64748B] text-xs">—</span>
                        )}
                        {editing ? (
                          <div className="flex items-center gap-1 mt-1">
                            <Input
                              ...
                              onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addRemark(r); } if (e.key === 'Escape') { setRemarkEditId(null); setRemarkDraft(''); } }}
                              placeholder="Add remark…"
                              className="h-7 text-xs"
                              disabled={remarkSaving}
                              data-testid={`sched-remark-input-${r.id}`}
                            />
                            <Button size="sm" className="h-7 px-2 text-xs" onClick={() => addRemark(r)} disabled={remarkSaving} data-testid={`sched-remark-save-${r.id}`}>{remarkSaving ? '…' : 'Add'}</Button>
                          </div>
                        ) : (
                          <button
                            type="button"
              ...
              return (
                <React.Fragment key={r.id}>
                  {dateChanged && (
                    <tr
                      aria-hidden="true"
                      data-testid={`date-separator-${r.date}`}
                    >
                      <td
                  ...
                  {visibleCols.map((c, i) => {
                    ...
                    return (
                      <td
                        key={c.key}
                        style={{ width: `${getColWidth(c.key)}px` }}
                        className={`${tdPad} ${cellBorder} ${borderOverride || ''} ${stickyClass} ${textOverride} overflow-hidden`}
          ...
          {!editing && (
            <div className="mb-4">
              ...
              <div className="inline-flex rounded-lg border bg-muted/30 p-1">
                <Button
                  type="button"
                  variant={scheduleMode === 'once' ? 'default' : 'ghost'}
                  size="sm"
                  className={scheduleMode === 'once' ? 'shadow-sm' : ''}
                  onClick={() => setScheduleMode('once')}
                  data-testid="schedule-mode-once"
                >
                  Once
                </Button>
                <Button
                  type="button"
                  variant={scheduleMode === 'multiple' ? 'default' : 'ghost'}
                  size="sm"
                  className={scheduleMode === 'multiple' ? 'shadow-sm' : ''}
                  onClick={() => setScheduleMode('multiple')}
                  data-testid="schedule-mode-multiple"
                >
                  Multiple
                </Button>
              </div>
              <p className="mt-1.5 text-xs text-muted-foreground">
                {scheduleMode === 'once'
                  ? 'Create one dispatch schedule for a single date.'
                  : 'Create the same shift for every date in the selected range.'}
              </p>
            </div>
          )}

          <div className="grid grid-cols-2 gap-3">
            {scheduleMode === 'multiple' && !editing ? (
              <>
                <div>
            ...
            <div>
              ...
              <Input
                type="time"
                value={form.start_time === 'UMA' ? '' : (form.start_time || '')}
                onChange={(e) =>
                  setForm({ ...form, start_time: e.target.value })
                }
                disabled={form.start_time === 'UMA'}
                data-testid="sf-start"
              />

              <label className="mt-2 flex items-center gap-2 text-xs text-muted-foreground cursor-pointer">
                <input
                  type="checkbox"
                  checked={form.start_time === 'UMA'}
                  onChange={(e) =>
                    setForm({
                      ...form,
                      start_time: e.target.checked ? 'UMA' : ''
                    })
                  }
                  data-testid="sf-start-uma"
                />
                <span>UMA</span>
              </label>
            </div>
            ...
            <div>
              ...
              <Input
                type="time"
                value={form.end_time === 'UMA' ? '' : (form.end_time || '')}
                onChange={(e) =>
                  setForm({ ...form, end_time: e.target.value })
                }
                disabled={form.end_time === 'UMA'}
                data-testid="sf-end"
              />
              ...
              <label className="mt-2 flex items-center gap-2 text-xs text-muted-foreground cursor-pointer">
                <input
                  type="checkbox"
                  checked={form.end_time === 'UMA'}
                  onChange={(e) =>
                    setForm({
                      ...form,
                      end_time: e.target.checked ? 'UMA' : ''
                    })
                  }
                  data-testid="sf-end-uma"
                />
                <span>UMA</span>
              </label>
            </div>
                  ...
                  <SearchableSelect
                    ...
                    onChange={(v) => {
                      ...
                      setForm({
                        ...
                        officer_id:
                          p?.client_id && String(p.client_id) !== String(form.client_id)
                            ? ''
                            : form.officer_id,
                      });
                    }}
                ...
                options={[
                  ...
                  ...(form.client_id
                    ? officers.filter(
                        o =>
                          o.status === 'active' &&
                          String(o.client_id) === String(form.client_id)
                      )
                    : []),
                ]}
                getLabel={(o) => o.name}
                getSearch={(o) => `${o.name} ${o.officer_code || ''} ${o.email || ''} ${o.contact_number || ''}`}
                placeholder={
                  form.client_id
                    ? "Search officers…"
                    : "Select a client first"
                }
                searchPlaceholder="Type officer name, code, email or phone"
                emptyText={
                  form.client_id
                    ? "No active officers for this client"
                    : "Select a client first"
                }
                testid="sf-officer"
              />
      ...
      {/* Inline "New Post Site" dialog — triggered from the + next to Post Site select */}
      <Dialog open={siteDialogOpen} onOpenChange={setSiteDialogOpen}>
        <DialogContent className="max-w-lg" data-testid="new-post-site-dialog">
          <DialogHeader>
            <DialogTitle>New Post Site</DialogTitle>
            <DialogDescription>Client &amp; Vendor are already picked in the schedule form above.</DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-3">
            <div><Label>Post Site Pin *</Label><Input value={siteForm.post_pin} onChange={(e) => setSiteForm({ ...siteForm, post_pin: e.target.value })} data-testid="ps-pin" /></div>
            <div><Label>Post Site Name *</Label><Input value={siteForm.name} onChange={(e) => setSiteForm({ ...siteForm, name: e.target.value })} data-testid="ps-name" /></div>
            <div><Label>Location *</Label><Input value={siteForm.location} onChange={(e) => setSiteForm({ ...siteForm, location: e.target.value })} data-testid="ps-location" /></div>
            <div><Label>City</Label><Input value={siteForm.city} onChange={(e) => setSiteForm({ ...siteForm, city: e.target.value })} data-testid="ps-city" /></div>
            <div>
              <Label>Type</Label>
              <Select
                value={siteForm.type || 'Unarmed'}
                onValueChange={(value) => setSiteForm({ ...siteForm, type: value })}
              >
                <SelectTrigger data-testid="ps-type">
            ...
            <div className="col-span-2"><Label>Note</Label><Textarea value={siteForm.notes} onChange={(e) => setSiteForm({ ...siteForm, notes: e.target.value })} data-testid="ps-notes" /></div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={importOpen} onOpenChange={(o) => { if (!importBusy) setImportOpen(o); }}>
        <DialogContent className="max-w-2xl" data-testid="import-csv-dialog">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload a CSV to create many schedules at once. Each row = one shift.
              Rows with errors are skipped and reported below — valid rows are still created.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4">
            <div className="rounded-lg border border-[#E2E8F0] dark:border-[#27272A] p-3 text-xs text-[#334155] dark:text-[#E4E4E7] space-y-2">
              <div className="font-semibold text-[#0F172A] dark:text-[#FAFAFA]">Required columns</div>
              <div className="font-mono text-[11px] bg-[#F8FAFC] dark:bg-[#0F0F11] p-2 rounded">
                date, shift_type, start_time, end_time, officer_name, post_pin
              </div>
              <div className="font-semibold text-[#0F172A] dark:text-[#FAFAFA]">Optional columns</div>
              <div className="font-mono text-[11px] bg-[#F8FAFC] dark:bg-[#0F0F11] p-2 rounded">
                work_order_number, officer_email, client_name, vendor_name, duty_rate, billing_rate, remarks
              </div>
              <ul className="list-disc pl-5 space-y-1">
                <li><b>date</b> in YYYY-MM-DD, <b>start_time</b>/<b>end_time</b> in HH:MM (24h).</li>
                <li><b>shift_type</b>: Morning · Afternoon · Evening · Night.</li>
                <li>Officer is matched by <b>officer_email</b> first, then by <b>officer_name</b>.</li>
                <li>Post site is matched by its <b>post_pin</b>. If the post has no client/vendor
                  attached, add <b>client_name</b> and <b>vendor_name</b> columns to fill them in.</li>
                <li>Rows that duplicate an existing shift (same officer + date + start time) are skipped, not treated as errors.</li>
                <li>Use <b>Preview</b> to validate without saving. Nothing is written until you click <b>Import CSV</b>.</li>
              </ul>
              <Button
                variant="outline"
                size="sm"
                onClick={downloadImportTemplate}
                data-testid="import-csv-download-template"
              >
            ...
            {importResult && (
              <div className="space-y-2" data-testid="import-csv-result">
                <div className="flex items-center gap-4 text-sm flex-wrap">
                  ...
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300">
                    {importResult.dry_run ? 'Ready to import' : 'Created'}: <b data-testid="import-created-count">{importResult.created}</b>
                  </span>
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300">
                    Skipped: <b data-testid="import-skipped-count">{importResult.skipped?.length ?? 0}</b>
                  </span>
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300">
                    Errors: <b data-testid="import-errors-count">{importResult.errors?.length ?? 0}</b>
                  </span>
                </div>

                {importResult.skipped && importResult.skipped.length > 0 && (
                  <div className="border border-[#E2E8F0] dark:border-[#27272A] rounded-lg overflow-hidden max-h-40 overflow-y-auto">
                    <table className="w-full text-xs" data-testid="import-skipped-table">
                      <thead className="bg-[#F8FAFC] dark:bg-[#0F0F11] text-[#64748B]">
                        <tr>
                          <th className="px-3 py-2 text-left w-16">Row</th>
                          <th className="px-3 py-2 text-left">Skipped because</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[#E2E8F0] dark:divide-[#27272A]">
                        {importResult.skipped.map((sk, i) => (
                          <tr key={i} data-testid={`import-skipped-row-${i}`}>
                            <td className="px-3 py-1.5 font-mono">{sk.row}</td>
                            <td className="px-3 py-1.5 text-amber-700 dark:text-amber-400">{sk.reason}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {importResult.errors && importResult.errors.length > 0 && (
                  <div className="border border-[#E2E8F0] dark:border-[#27272A] rounded-lg overflow-hidden max-h-64 overflow-y-auto">
                    <table className="w-full text-xs">
                      ...
                      <tbody className="divide-y divide-[#E2E8F0] dark:divide-[#27272A]">
                        {importResult.errors.map((er, i) => (
                          <tr key={i} data-testid={`import-error-row-${i}`}>
                            <td className="px-3 py-1.5 font-mono">{er.row}</td>
                            <td className="px-3 py-1.5 text-rose-700 dark:text-rose-400">{er.message}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                options={
                  filters.client_id
                    ? officers.filter(
                        (o) =>
                          o.status === 'active' &&
                          String(o.client_id) === String(filters.client_id)
                      )
                    : officers
                }
                getLabel={(o) => o.name}
            ...
            <div><Label className="text-xs">Client</Label>
              <SearchableSelect
                ...
                onChange={(v) => {
                  ...
                  setFilters((prev) => {
                    const selectedOfficer = prev.officer_id
                      ? officers.find(
                          (o) => String(o.id) === String(prev.officer_id)
                        )
                      : null;

                    const officerBelongsToClient =
                      selectedOfficer &&
                      String(selectedOfficer.client_id) === String(v);

                    return {
                      ...prev,
                      client_id: v,
                      officer_id: officerBelongsToClient
                        ? prev.officer_id
                        : '',
                    };
                  });
      ...
      <Dialog open={chooserOpen} onOpenChange={setChooserOpen}>
        <DialogContent className="max-w-md" data-testid="column-chooser-dialog">
          ...
          {/* Density toggle */}
          <div className="flex items-center justify-between rounded-lg border border-[#E2E8F0] dark:border-[#27272A] p-2" data-testid="density-toggle-row">
            <div className="text-sm">
              <div className="font-medium text-[#0F172A] dark:text-[#FAFAFA]">Row density</div>
              <p className="text-xs text-[#64748B]">Tighten row height and font size for scanning long lists.</p>
            </div>
            <div className="inline-flex rounded-md border border-[#E2E8F0] dark:border-[#27272A] overflow-hidden">
              <button
                type="button"
            ...
            {columnConfig.map((c) => {
              ...
              return (
                <li
                  key={c.key}
                  draggable
                  onDragStart={(e) => { setDragKey(c.key); try { e.dataTransfer.effectAllowed = 'move'; } catch (err) { console.warn('dataTransfer.effectAllowed unavailable', err); } }}
