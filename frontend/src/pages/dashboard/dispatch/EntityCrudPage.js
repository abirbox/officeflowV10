...
const EntityCrudPage = ({ title, endpoint, permBase, fields, columns, statuses, filters = [], rowActions }) => {
  ...
  useEffect(() => {
    load();
    if (fields.some((f) => f.select === 'clients' || f.multi === 'clients')) api.get('/dispatch/clients').then(r => setClients(r.data));
  ...
  const remove = async (row) => {
    if (!window.confirm(`Permanently delete "${row.name || row.post_pin}"? This cannot be undone.`)) return;
  ...
  const renderField = (f) => {
    ...
    if (f.multi === 'clients') {
      ...
      const toggle = (id) => setForm({
        ...form,
        [f.key]: selected.includes(id) ? selected.filter((x) => x !== id) : [...selected, id],
      });
      return (
        <div className="border border-[#E2E8F0] dark:border-[#27272A] rounded-lg max-h-44 overflow-y-auto divide-y divide-[#E2E8F0] dark:divide-[#27272A]" data-testid={`field-${f.key}`}>
          {clients.length === 0 ? (
    ...
    if (f.type === 'color') {
      const val = form[f.key] || '#D4A017';
      return (
        <div className="flex items-center gap-2">
          <Input type="color" value={val}
            onChange={(e) => setForm({ ...form, [f.key]: e.target.value })}
            className="w-16 h-10 p-1 cursor-pointer" data-testid={`field-${f.key}`} />
          <Input type="text" value={form[f.key] || ''} placeholder="#D4A017 (default gold)"
            onChange={(e) => setForm({ ...form, [f.key]: e.target.value })}
            className="w-32 font-mono uppercase" data-testid={`field-${f.key}-hex`} />
          {form[f.key] && (
            <Button size="sm" variant="ghost" className="text-xs" onClick={() => setForm({ ...form, [f.key]: '' })} data-testid={`field-${f.key}-reset`}>Reset</Button>
          )}
        </div>
      );
    }
